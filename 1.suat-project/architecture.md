...
> **Proje:** Suat Ak Kahve Filtre S2P  
...
| Domain | n8n.suatak.forenly.ai (Caddy reverse proxy) |
...
services:
  n8n:
    ...
    ports:
      - "5678:5678"
    environment:
      - N8N_HOST=n8n.suatak.forenly.ai
      - N8N_PROTOCOL=https
      - WEBHOOK_URL=https://n8n.suatak.forenly.ai/
      - DB_TYPE=postgresdb
  ...
  cloud-sql-proxy:
    ...
    command: "s2p-suatak-prod:europe-west1:s2p-db"
    restart: always
...
| Bucket | Amaç |
|---|---|
| `s2p-suatak-invoices` | Gelen PDF faturalar |
| `s2p-suatak-contracts` | Tedarikçi sözleşmeleri |
| `s2p-suatak-exports` | Raporlar & export dosyaları |
...
7. [MANUAL] Suat Ak onaylar → status = "approved"
...
9. [DECISION]
   ...
   - verdict = "review_required" → Suat Ak'a bildirim
...
> 💡 Suat Ak'a sunumda: "Aylık ~$50 altyapı maliyeti ile 4 saat/fatura × 100 fatura = 400 saat/ay'lık manuel çalışmayı eliminate ediyoruz."

---

## 6. Deployment Checklist

- [ ] GCP Project oluştur: `s2p-suatak-prod`
...
- [ ] DNS ayarları: `n8n.suatak.forenly.ai`
